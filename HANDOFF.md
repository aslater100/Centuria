...
- ◑ **Leg 1 — world market: SUBSTANTIALLY COMPLETE.** ✅ `worldGoodPrice`/`worldGoodSupply`/`worldGoodDemand`/
  ...
  **Telemetry-only activation: the world SIGNAL is now global & alive; the on-map economy does not yet RESPOND.**
  ...
  bounded/solvent. **STILL DEFERRED (the next macro bite + re-baseline):** feed `finalConsumptionShortfall` into the
  MACRO coupling — the industry output-drag (`LOCAL_GOODS_OUTPUT_DRAG`, UNgated → would bite immediately) and/or
  cost-push (central-bank-gated → needs the autoplay player to reach statehood first) — and flip `consumerDemand` ON
  by default; both want a human downturn playtest. (`localGoodsScarcity` — the input-stranding index — stays 0 because
  the sink drives FINAL-consumption shortage, not production-input starvation.)
