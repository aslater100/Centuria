...
**Last updated:** 2026-07-01 (session 15 — the World's Arsenals: rival wars scarcity-decided, wars drain the arsenals, the space race) · **Tests:** 1389 passing · **Version:** v1.5.0 · **Status:** Phases 1–18 complete; deep-expansion underway. **LATEST SESSION (session 15 — THE WORLD'S ARSENALS; economy-realism arc inc 3; 3 feature commits + handoff):** the full write-up lives in `.handoff.md` (top entry) — summary: (1) `rivalArmsCapacity`/`rivalWarWinChance` make the abstract rival↔rival wars ARMS-decided (pop weighted by the same FLOOR+(1−FLOOR)×capacity equip curve as `warPower()`), flag-gated on `rivalClimateResponse` with an IEEE-exact ×1.0 cancellation so the default sweep is BYTE-IDENTICAL (diffed + independently A/B-proven by a verify skeptic); the climate sweep re-baselines boundedly (same rng draw count → 5/8 seeds byte-identical). (2) `consumeWarMateriel` closes the military-industrial loop: fielded unit power draws real steel/chemicals from town `goodStocks` monthly via `drawGood` (first live caller), shortfall saps morale (floor 30) which `warPower()` reads; playerWar-gated → every sweep byte-identical. (3) Phase D space wonders (worktree-agent parallel build, pure data+tests): `rocketry`(1955)/`satellites`(1965) tech ladder + `space_program`/`satellite_network`/`orbital_station` uniques — rivals claimed 3/3 in probe runs (space_program ~1966); HEALTHY default re-baseline (rival wonder race: seed 1000 dystopia→drowned is the only default outcome flip; wars still 0). NEW PINNED SWEEPS: default 3 drowned/5 dystopia; climate wars 21–52, 5 drowned/3 dystopia; combined climate+consumer wMkt 21.5–27.2%, wars 19–53, 3 drowned/5 dystopia — the KEY verify finding: the scarcity CHOKE on rival arms procurement is inert under the climate flag alone (legacy `worldGoodScarcity` pinned 0) and goes live only with `SIM_CONSUMER_DEMAND=1` (same latent dependency as `rivalResourceGrievance`). Gates: tsc clean, 1389 tests (+10 rival-arms, +7 war-materiel, +7 space-wonders, techtree count 56→58), serialize-determinism BYTE-IDENTICAL, save-size green, bench PASS (worst 11.2ms — run bench SOLO: concurrent sweeps false-FAIL it), verify-Workflow 3/3 refuted=false.
